...
public sealed partial class SettingsPage : Page
{
    ...
    private void OnRestore(object sender, RoutedEventArgs e) { var settings = _settingsService.Update(current => { current.Theme = "System"; current.DecimalPlaces = 2; current.AutoCalculate = false; }); Apply(settings); StatusText.Text = "已恢复默认设置。"; }
    private void OnSettingChanged(object sender, object e) { if (!_isApplying) SaveCurrentSettings(); }
    private void SaveCurrentSettings() { var settings = _settingsService.Update(current => { current.Theme = (ThemeBox.SelectedItem as ComboBoxItem)?.Tag?.ToString() ?? "System"; current.DecimalPlaces = DecimalBox.SelectedIndex < 0 ? 2 : DecimalBox.SelectedIndex; current.AutoCalculate = AutoCalculateToggle.IsOn; }); ApplyTheme(settings.Theme); StatusText.Text = "设置已保存。"; }
