...
UrbanPlanToolbox はアカウントを要求せず、個人情報を収集・販売・アップロードしません。プロジェクト、設定、お気に入り、ツールデータ、マイルストーン、通知関連データ、アプリが管理する添付ファイルは `%LocalAppData%\\UrbanPlanToolbox` に保存されます。ユーザーが選択した作業フォルダーは参照と OS のアクセス トークンだけを保持し、外部フォルダーをコピー、スキャン、削除しません。

Microsoft Store 版の更新は Microsoft Store が管理します。GitHub サイドロード版が GitHub Releases API に接続するのは、ユーザーが更新確認を明示的に実行した場合だけです。プロジェクト、Issue、公式法規、サポート、プライバシー、Store のページはクリックしたときだけ開きます。広告、テレメトリ、追跡、自動クラッシュ送信、自動ログ送信、ユーザーデータの自動送信はありません。外部サイトには各自のプライバシーポリシーがあります。
