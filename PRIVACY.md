...
Version 1.9.2 · Last updated 2026-08-19

## 简体中文

UrbanPlanToolbox 不要求应用账户，不收集、出售用户个人信息，不包含广告、遥测、追踪、自动崩溃上传或自动日志上传。项目、设置、收藏、工具数据、时间节点、通知数据、应用管理的附件以及已安装的 `.uptdata` 专业资料包默认保存在本机 `%LocalAppData%\UrbanPlanToolbox`。
...
用户选择的工作文件夹只保存引用和系统访问令牌；应用不会自动复制、扫描或删除外部文件夹。Shapefile 等坐标文件只在设备本机读取和转换，不会因 WebDAV 功能自动上传。法规库、规划术语库和设计理念词典仅在用户主动执行“检查数据更新/下载并安装”时访问 `KiYouJyo/UrbanPlanToolbox_Data` 的 Catalog 与 GitHub Release 资产；本地 `.uptdata` 导入不需要上传任何文件。应用不会向该数据仓库上传用户项目、搜索内容或个人信息。Microsoft Store 版在应用更新相关操作时访问 Microsoft Store 服务；GitHub 旁加载版在应用更新相关操作时访问 GitHub Releases API。项目、政策、资料来源和支持链接只在用户明确操作后由系统默认浏览器打开。

## 日本語

UrbanPlanToolbox はアプリ独自のアカウントを要求せず、個人情報を収集・販売しません。広告、テレメトリ、トラッキング、自動クラッシュ送信、自動ログ送信もありません。プロジェクト、設定、お気に入り、ツールデータ、マイルストーン、通知データ、アプリ管理の添付ファイル、インストール済み `.uptdata` 専門データパックは既定で `%LocalAppData%\UrbanPlanToolbox` に保存されます。
...
ユーザーが選択した作業フォルダーは参照と OS のアクセス トークンだけを保持し、外部フォルダーを自動コピー、スキャン、削除しません。Shapefile などの座標ファイルは端末上でのみ処理され、WebDAV 機能によって自動アップロードされません。法規、都市計画用語、デザインコンセプトの各ライブラリは、ユーザーがデータ更新の確認またはダウンロードを明示的に実行した場合のみ `KiYouJyo/UrbanPlanToolbox_Data` の Catalog と GitHub Release アセットへ接続します。ローカル `.uptdata` の読み込みでファイルをアップロードすることはありません。ユーザープロジェクト、検索内容、個人情報をデータリポジトリへ送信しません。Microsoft Store 版はアプリ更新操作時に Store サービスへ、GitHub サイドロード版はアプリ更新操作時に GitHub Releases API へ接続します。

## English

UrbanPlanToolbox requires no app-specific account and does not collect or sell personal information. It has no ads, telemetry, tracking, automatic crash upload, or automatic log upload. Projects, settings, favorites, tool data, milestones, notification data, app-managed attachments, and installed `.uptdata` professional data packs are stored locally by default under `%LocalAppData%\UrbanPlanToolbox`.
...
A selected work folder is retained only as a reference and OS access token; the app does not automatically copy, scan, or delete the external folder. Coordinate datasets such as Shapefiles are processed locally and are not automatically uploaded by the WebDAV feature. The regulations, planning terminology, and design-concepts libraries contact the `KiYouJyo/UrbanPlanToolbox_Data` catalog and GitHub Release assets only when the user explicitly checks for or installs a data update. Importing a local `.uptdata` pack does not upload that file. The app does not send projects, search queries, or personal information to the data repository. The Microsoft Store edition contacts Microsoft Store services for app-update actions, while the GitHub sideload edition contacts the GitHub Releases API for app-update actions. Project, policy, source, and support links open only after explicit user action.
