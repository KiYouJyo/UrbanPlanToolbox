...
public sealed class VisualPolishPackagingTests
{
    ...
    public void VersionAndUserAgentAre110()
    {
        ...
        Assert.Contains("Version=\"1.5.5.0\"", File.ReadAllText(Path.Combine(root, "Package.appxmanifest")));
        Assert.Contains("<Version>1.5.5</Version>", File.ReadAllText(Path.Combine(root, "UrbanPlanToolbox.csproj")));
