...
public sealed class VisualPolishPackagingTests
{
    ...
    {
        var root = FindRepositoryRoot();
        Assert.Contains("Version=\"1.7.1.0\"", File.ReadAllText(Path.Combine(root, "Package.appxmanifest")));
        Assert.Contains("<Version>1.7.1</Version>", File.ReadAllText(Path.Combine(root, "UrbanPlanToolbox.csproj")));
