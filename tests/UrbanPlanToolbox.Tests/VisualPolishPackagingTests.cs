...
public sealed class VisualPolishPackagingTests
{
    ...
    {
        var root = FindRepositoryRoot();
        Assert.Contains("Version=\"1.7.3.0\"", File.ReadAllText(Path.Combine(root, "Package.appxmanifest")));
        Assert.Contains("<Version>1.7.3</Version>", File.ReadAllText(Path.Combine(root, "UrbanPlanToolbox.csproj")));
