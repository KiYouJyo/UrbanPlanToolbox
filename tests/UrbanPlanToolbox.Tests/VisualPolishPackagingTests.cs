...
public sealed class VisualPolishPackagingTests
{
    ...
    {
        var root = FindRepositoryRoot();
        Assert.Contains("Version=\"1.6.9.0\"", File.ReadAllText(Path.Combine(root, "Package.appxmanifest")));
        Assert.Contains("<Version>1.6.9</Version>", File.ReadAllText(Path.Combine(root, "UrbanPlanToolbox.csproj")));
