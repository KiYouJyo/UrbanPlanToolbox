...
public sealed class VisualPolishPackagingTests
{
    ...
    public void VersionAndUserAgentAre110()
    {
        ...
        Assert.Contains("Version=\"1.5.4.0\"", File.ReadAllText(Path.Combine(root, "Package.appxmanifest")));
        Assert.Contains("<Version>1.5.4</Version>", File.ReadAllText(Path.Combine(root, "UrbanPlanToolbox.csproj")));
