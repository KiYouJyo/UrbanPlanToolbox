...
public sealed partial class LocalizationTests
{
    ...
    public void SearchIndexUsesCurrentLanguageText()
    {
        ...
        Assert.Equal(ToolIds.PlanningIndicatorCalculator, Assert.Single(Flatten(zh.Search("规划", _ => false))).Id);
        Assert.Equal(ToolIds.PlanningIndicatorCalculator, Assert.Single(Flatten(ja.Search("容積率", _ => false))).Id);
        Assert.Equal(ToolIds.PlanningIndicatorCalculator, Assert.Single(Flatten(en.Search("planning", _ => false))).Id);
    ...
    public void SearchResultsReturnSameStableToolIdsAcrossLanguages()
    {
        foreach (var language in ReswCatalog.Languages)
        {
            ...
            Assert.Equal([ToolIds.UnitScaleConverter, ToolIds.PlanningIndicatorCalculator], ids);
        }
    }
    ...
    public void VersionConfigurationIs03111()
    {
        ...
        Assert.Contains("<Version>0.3.11.1</Version>", project);
        Assert.Contains("<AssemblyVersion>0.3.11.1</AssemblyVersion>", project);
        Assert.Contains("<FileVersion>0.3.11.1</FileVersion>", project);
