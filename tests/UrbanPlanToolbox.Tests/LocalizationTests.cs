...
public sealed partial class LocalizationTests
{
    ...
    public void SearchResultsReturnSameStableToolIdsAcrossLanguages()
    {
        foreach (var language in ReswCatalog.Languages)
        {
            ...
            Assert.Equal([ToolIds.UnitScaleConverter, ToolIds.DesignConceptDictionary, ToolIds.PlanningIndicatorCalculator, ToolIds.WorkflowReviewChecklist, ToolIds.ColorPaletteRecorder, ToolIds.RegulationsIndex, ToolIds.CoordinateSystemConverter], ids);
        }
    }
    ...
    public void VersionConfigurationIs110AndChannelsRemainDistinct()
    {
        ...
        Assert.Contains("Version=\"1.4.0.0\"", manifest);
        ...
        Assert.Contains("<Version>1.4.0</Version>", project);
        Assert.Contains("<AssemblyVersion>1.4.0</AssemblyVersion>", project);
        Assert.Contains("<FileVersion>1.4.0</FileVersion>", project);
        Assert.Contains("<InformationalVersion>1.4.0</InformationalVersion>", project);
        ...
        Assert.Contains("Version=\"1.4.0.0\"", storeManifest);
