...
foreach ($required in @("tags:", "'v*.*.*'", 'workflow_dispatch:', 'dry_run:', 'contents: write', 'refs/tags/v', 'BD85AD77A651C86CA01A480C8E9BC64952993F98', "GetEnvironmentVariable('ProgramFiles(x86)')", 'timestamp.digicert.com', 'certutil.exe -user -addstore -f Root', 'verify /pa /all /v', 'Create or reconcile GitHub Release', 'gh release create', '--notes-file', 'actions/upload-artifact@v4')) {
...
if ($text -match 'Import-Certificate') { throw 'Workflow must use certutil for explicit certificate-store exit handling.' }
