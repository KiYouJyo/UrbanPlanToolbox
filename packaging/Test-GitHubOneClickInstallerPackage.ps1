...
if ([int]$metadata.schemaVersion -ne 3 -or $metadata.displayVersion -ne '1.5.7' -or $metadata.packageVersion -ne '1.5.7.0') { throw 'One-click metadata version mismatch.' }
...
if ($metadata.releaseApiUri -ne 'https://api.github.com/repos/KiYouJyo/UrbanPlanToolbox/releases/latest' -or $metadata.releaseTag -ne 'v1.5.7' -or $metadata.checksumFileName -ne 'SHA256SUMS.txt') { throw 'One-click metadata does not describe the GitHub Release asset flow.' }
