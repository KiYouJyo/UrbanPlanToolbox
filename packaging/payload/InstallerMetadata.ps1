...
function Get-InstallerReleaseNames([string]$DisplayVersion, [string]$PackageVersion) {
    ...
    [pscustomobject]@{ ReleaseDirectoryName="UrbanPlanToolbox-v$DisplayVersion-x64-framework-dependent-self-signed"; MsixFileName="UrbanPlanToolbox_$PackageVersion`_x64_framework-dependent_self-signed.msix"; CertificateFileName="UrbanPlanToolbox-v$DisplayVersion-Framework-Dependent-Preview-Test.cer" }
}
...
function Get-MsixPackageMetadata([string]$MsixPath) {
    Add-Type -AssemblyName System.IO.Compression.FileSystem; $archive=[IO.Compression.ZipFile]::OpenRead($MsixPath)
    try { $entry=$archive.Entries|Where-Object {$_.FullName -ieq 'AppxManifest.xml'}|Select-Object -First 1; if($null -eq $entry){throw 'MSIX missing AppxManifest.xml.'}; $reader=[IO.StreamReader]::new($entry.Open()); try{[xml]$xml=$reader.ReadToEnd()}finally{$reader.Dispose()} } finally {$archive.Dispose()}
