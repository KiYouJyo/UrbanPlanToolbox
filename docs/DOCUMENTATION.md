...
当前阶段不以新增产品功能为目标。维护版本推进必须先完成版本事实源、Release Notes、CHANGELOG 与发布元数据的一致性校验，再进入验证与明确发布审批；文档治理继续消除陈旧版本状态、旧 updater 描述、过时 Store 里程碑规则和版本绑定式专题说明，并为后续自动生成 Release Notes、网站版本历史及发布元数据建立清晰边界。
