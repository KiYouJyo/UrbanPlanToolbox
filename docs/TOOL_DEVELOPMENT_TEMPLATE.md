...
`color-palette-recorder` is the first real implementation of this template. `workflow-review-checklist` is the second real sample and validates a pure structured-data tool with multiple placements. Both schemas are intentionally separate from `ProjectSchemaVersion`; the portable container remains `BackupFormatVersion = 1`.
