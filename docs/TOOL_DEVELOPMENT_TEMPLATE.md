...
1. Assign an immutable lowercase stable tool ID and select primary/secondary categories.
...
4. Verify the category card, search index, and `FavoriteToolsService` operate on the stable ID.
...
`color-palette-recorder` is the first real implementation of this template. Its schema is intentionally separate from `ProjectSchemaVersion`; the portable container remains `BackupFormatVersion = 1`.
