...
计划中的 Microsoft Store 首次提交包使用 `0.5.0.0`，必须在 Partner Center 关联正式 Identity 和 Publisher 后重新构建。当前仓库尚未关联正式商店身份；`CN=AppPublisher` 只用于自签名旁加载渠道。GitHub 侧载版和 Microsoft Store 版保持独立发行身份及更新渠道。
