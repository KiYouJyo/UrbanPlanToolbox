...
> 当前开发完成版本：`0.4.0`（MSIX `0.4.0.0`），等待 Draft PR #17 后续合并与发布。v0.4.0 新增色卡方案记录器：本地保存色卡方案、多个独立颜色与应用内部图片附件，并支持三语、备份导入导出和响应式编辑。
...
- 未来的重要时间节点可逐项启用 Windows 本地提醒；未填写具体时间时默认在本地时间 09:00 提醒。提醒随编辑、归档、删除、导入、语言切换和启动自动对账；详见 [项目时间节点提醒](docs/MILESTONE_REMINDERS.md)。
