...
public sealed class TrayService : IDisposable
{
    ...
    public void Initialize()
    {
        ...
        _icon = LoadThemeAwareIcon(); var data = Data(Message | Icon | Tip); if (!ShellNotifyIcon(Add, ref data)) throw new InvalidOperationException("Cannot create the tray icon."); _created = true; _uiSettings.ColorValuesChanged += OnSystemColorsChanged;
    }
    ...
    {
        var replacement = LoadThemeAwareIcon(); if (replacement == 0) return;
        var previous = _icon; _icon = replacement;
        if (_created) { var data = Data(Icon); ShellNotifyIcon(Modify, ref data); }
    ...
    private void Menu()
    {
        var t = LocalizationService.Default; var menu = CreatePopupMenu(); AppendMenu(menu, String, Open, t.GetString("Tray_Open")); AppendMenu(menu, Separator, 0, null); AppendMenu(menu, String | (_recorderVisible ? Checked : 0), Recorder, t.GetString("Tray_Recorder")); AppendMenu(menu, Separator, 0, null); AppendMenu(menu, String, Settings, t.GetString("Tray_Settings")); AppendMenu(menu, String, Exit, t.GetString("Tray_Exit")); GetCursorPos(out var p); SetForegroundWindow(_window); TrackPopupMenu(menu, RightButton, p.X, p.Y, 0, _window, 0); DestroyMenu(menu);
    }
    ...
    public void Dispose() { if (_window == 0) return; _uiSettings.ColorValuesChanged -= OnSystemColorsChanged; var data = Data(0); ShellNotifyIcon(Delete, ref data); Owners.Remove(_window); DestroyWindow(_window); if (_icon != 0) DestroyIcon(_icon); _window = 0; _created = false; }
