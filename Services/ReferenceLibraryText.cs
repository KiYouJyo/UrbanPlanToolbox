...
public static class ReferenceLibraryText
{
    private static readonly IReadOnlyDictionary<string, string[]> Values = new Dictionary<string, string[]>(StringComparer.Ordinal)
    {
        ...
        ["CloudNotChecked"] = ["云端版本：尚未检查", "クラウド版：未確認", "Cloud version: not checked"],
        ...
        ["CatalogNetworkUnavailable"] = ["无法连接官方数据目录。已尝试 raw.githubusercontent.com 与 GitHub API；请检查网络或代理后重试。", "公式データカタログに接続できません。raw.githubusercontent.com と GitHub API の両方を試しました。ネットワークまたはプロキシを確認してください。", "Could not reach the official data catalog. Both raw.githubusercontent.com and the GitHub API were tried; check the network or proxy and retry."],
        ["CatalogInvalid"] = ["官方数据目录已连接，但返回内容无法通过 Data Pack 1.0 校验。请稍后重试或查看日志。", "公式データカタログには接続できましたが、返された内容が Data Pack 1.0 の検証に失敗しました。後でもう一度試すかログを確認してください。", "The official data catalog was reached, but its response failed Data Pack 1.0 validation. Retry later or inspect the log."],
        ["CatalogMissingPack"] = ["官方数据目录可访问，但没有找到当前数据库对应的数据包。", "公式データカタログには接続できましたが、このデータベース用のデータパックが見つかりません。", "The official data catalog is reachable, but it does not contain a pack for this database."],
